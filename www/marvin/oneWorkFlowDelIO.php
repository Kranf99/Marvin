...
if ($isSuperAdmin) {
    // Apply immediately
    $stmt = $db->prepare('DELETE FROM workflowIO WHERE idWorkflow=:idwf AND idIO=:idio AND isInput=:isi');
...
if ($existing) {
    $stmtU = $db->prepare(
        "UPDATE workflowIOChanges SET changeType='remove', isInput=:isi, updatedAt=:now WHERE changeId=:cid"
    );
    ...
    $stmtI = $db->prepare(
        "INSERT INTO workflowIOChanges (idWorkflow, idIO, isInput, changeType, changedByUserId, isSuperAdmin, changeStatus, createdAt, updatedAt)
         VALUES (:idwf, :idio, :isi, 'remove', :uid, :admin, :cs, :now, :now)"
    );
